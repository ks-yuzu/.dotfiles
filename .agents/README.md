...
既適用の判定には `test -e` ではなく `test -L` を使う。実ファイルが置かれている場合、
`test -L` なら `ln -s` が失敗して止まるが、`test -e` はそれを既設と見なして黙って抜ける。

再測する場合は HOME を隔離したディレクトリへ向け、手順を 2 回実行する。
3 本の link が張られ、2 回目で差分が出ないことを見る。
